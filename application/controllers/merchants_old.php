...
class Merchants extends My_Controller {
    ...
    public function __construct() {
        ...
        parent::__construct();
        //echo base_url();
        $allowed_urls = array('index', 'dashboard');
    ...
    /* public function index() {
      ...
      if (!empty($result_s)) {
      $this->session->set_userdata('merchant_id', $result->id);
    ...
    public function index() {
        ...
        $where = "DATE(datecreated) > DATE_SUB(CURDATE(), INTERVAL 7 DAY) AND user_id = " . $this->session->userdata('merchant_id');
        ...
        $where = "id_user = " . $this->session->userdata('merchant_id');
    ...
    public function getfollowers() {
        ...
        $where = array('F.id_user ' => $this->session->userdata('merchant_id'));
    ...
    public function getemaillist() {
        ...
        $where = array('user_id ' => $this->session->userdata('merchant_id'));
        $data = $this->common_model->getTable($aColumns, $where, $col = 0, $order = 'desc', $groupby = NULL);
        $output = $data['output'];
        $j = $this->input->get_post('iDisplayStart', true) + 1;
        foreach ($data['result'] as $aRow) {
